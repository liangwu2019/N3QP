...
void N3_BoxQP_dsyr(double *H, double *h, double epsilon, double alpha, double delta, int n, double *z)
{
	int i, j, k, iter, info;
    ...
    else
    {
    	...
    	info = LAPACKE_dpotrf(LAPACK_COL_MAJOR,'L',n,M,n);
        info = LAPACKE_dpotri(LAPACK_COL_MAJOR,'L',n,M,n);
        ...
        {     
        	...
        	for(j=0;j<=count;j++)
        	{
        		...
        		cblas_dsyr(CblasColMajor,CblasLower,n,coeff,temp,ione,M,n);
                d[i] = temp1[i]+temp2[i];
        	}
        	cblas_dsymv(CblasColMajor,CblasLower,n,fone,M,n,delta_z_temp,ione,fzero,delta_z,ione);
